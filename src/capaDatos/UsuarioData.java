...
public Usuario getUsuario(Usuario u) throws SQLException, ApplicationException
{
  ...
  Usuario ub=null;
  try {
	  	stmt=FabricaDeConexion.getFabrica().getConexion().prepareStatement("select idUsuario,us.idPlan as idPlan,p.cantPeliculas as cantPeliculas,feContratacionServicio,estado from usuarios us inner join planes p on us.idPlan=p.idPlan where mail=? and password=?");
	  	...
	  	while(rs.next())
	  	{
	  		...
	  		ub.setFeContratacion(rs.getString("feContratacionServicio"));
	  		Plan p=new Plan();
